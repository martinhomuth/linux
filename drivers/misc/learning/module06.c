#include <linux/module.h>
#include <linux/init.h>
#include <linux/fs.h>
#include <linux/cdev.h>
#include <linux/device.h>
#include <linux/uaccess.h>
#include <linux/learning/module06.h>

static dev_t dev_num;
static struct cdev *device_object;
static struct class *class;
static struct device *module06_dev;
#define DRIVER_NAME "module06"

static int device_open(struct inode *inode, struct file *filep)
{
	dev_info(module06_dev, "%s opened by %d with mode %x ", DRIVER_NAME,
		 inode->i_uid.val, filep->f_flags);

	/* perform some initialization */

	return 0;
}

static int device_close(struct inode *inode, struct file *filep)
{
	dev_info(module06_dev, "%s closed by %d", DRIVER_NAME, inode->i_uid.val);
	/* perform some cleanup */

	return 0;
}

static long device_ioctl(struct file *filep, unsigned int cmd,
			 unsigned long arg)
{
	unsigned not_copied;
	dev_info(module06_dev, "ioctl called 0x%4.4lx %p\n", (long) cmd, (void *)arg);

	switch (cmd) {
	case MODULE06_GETVALUE:
		not_copied = copy_to_user((void *)arg, "IOCTL value", 11);
		break;
	default:
		dev_warn(module06_dev, "Unknown IOCTL 0x%lx", (long) cmd);
		return -EINVAL;
	}
	return 0;
}

static struct file_operations fops = {
	.owner = THIS_MODULE,
	.open = device_open,
	.release = device_close,

	/* long (*compat_ioctl) (struct file *, unsigned int, unsigned
	   long); */
	.unlocked_ioctl = device_ioctl,
};

int __init module06_initialize(void)
{
	if (alloc_chrdev_region(&dev_num, 0, 1, DRIVER_NAME)) {
		pr_err("Unable to allocate device number for device");
		return -EIO;
	}

	device_object = cdev_alloc();
	if (device_object == NULL) {
		pr_err("Unable to allocate character device object");
		goto free_device_number;
	}
	device_object->owner = THIS_MODULE;
	device_object->ops = &fops;
	if (cdev_add(device_object, dev_num, 1)) {
		pr_err("Unable to add character device object");
		goto free_cdev;
	}

	class = class_create(THIS_MODULE,
			     DRIVER_NAME);
	if (IS_ERR(class)) {
		pr_err("Unable to create class object for %s", DRIVER_NAME);
		goto free_cdev;
	}
	module06_dev = device_create(class, NULL, dev_num, NULL, "%s", DRIVER_NAME);
	if (IS_ERR(module06_dev)) {
		pr_err("module06: device_create failed\n");
		goto free_class;
	}

	return 0;
 free_class:
	class_destroy(class);
 free_cdev:
	kobject_put(&device_object->kobj);
 free_device_number:
	unregister_chrdev_region(dev_num, 1);
	return -EIO;
}

void __exit module06_cleanup(void)
{
	device_destroy(class, dev_num);
	class_destroy(class);
	cdev_del(device_object);
	unregister_chrdev_region(dev_num, 1);
}

module_init(module06_initialize);
module_exit(module06_cleanup);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Martin Homuth");
MODULE_DESCRIPTION("Module06 - ioctl demonstration");

